...
# subs = ['06', '13', '16', '17', '18', '21'] # left MTG patients
subs = ['07', '10', '12', '15', '16'] # right MTG patients
...
for sub in subs:
    ...
    file_paths[sub] = {
        ...
        'out_path_metrics': f'Metrics/{brain_region}/{prediction_variable}/RandomForest{event}/Subject{sub}',
        'out_path_plots': f'Plots/{brain_region}/{prediction_variable}/RandomForest{event}/Subject{sub}'
    }
## %%
...
for sub in subs:
    ...
    brain_area = 'middle temporal gyrus R'
    channel_idxs = np.where(elec_areas == brain_area)[0]

    data = np.load(file_paths[sub]['data_path'])[:,channel_idxs,:,40:100] # 40:100 is the time window of interest for visual cue as event
    ...
    file_name = f'Metrics/{brain_region}/{prediction_variable}/RandomForest{event}/ChannelInfo/Subject{sub}_{brain_area}_info.csv'
    ...
    #     plot_freq_band_data_for_channel(f_band_data, y, ch, time_window, event_delay=0, out_path_plots=out_path_plots, elec_names=elec_names, elec_areas=elec_areas)
    ...
    # clf.plot_heatmap(top_chs, event_delay=0, sub=sub, top_accuracy=accuracies[peak_accuracy_group_idx], optimal_combination=False, out_path=out_path_plots)
    ...
    clf.plot_heatmap(optimal_channel_combination, event_delay=0, sub=sub, top_accuracy=peak_accuracy, optimal_combination=True, out_path=out_path_plots)

...
for sub in subs:
    ...
    file_paths[sub] = {
        ...
        'out_path_metrics': f'Metrics/Subject{sub}_logistic_visual_card',
        'out_path_plots': f'Plots/Subject{sub}_logistic_visual_card'
    }
## %%
...
for sub in subs:
    ...
    card_categories = []
    idxs_to_keep = [] # only keep trials when subject draws a card other than 6

    # 0 = low card (2,4), 1 = high card (8, 10)
    ...
    data = np.load(file_paths[sub]['data_path'])[idxs_to_keep,:,:,40:100] # 40:100 is the time window of interest for visual cue as event
    y = np.asarray(card_categories)
    ...
    for optimal_time_windows_info in clf._optimal_time_windows_per_channel[:10]:
        ...
        plot_freq_band_data_for_channel(f_band_data, y, ch, time_window, event_delay=3, out_path_plots=out_path_plots, elec_areas=elec_areas)
    ...
    clf.plot_heatmap(results[0][0], 0, results[0][1], True, out_path_plots)
